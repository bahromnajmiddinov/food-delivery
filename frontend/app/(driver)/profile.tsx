...
import { ShieldCheck, Settings, MapPin, List, MessageCircle, RefreshCw } from 'lucide-react-native'
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.infoRow}>
        <View style={styles.infoCard}>
          <Text style={styles.infoTitle}>Active orders</Text>
          <Text style={styles.infoValue}>3</Text>
        </View>
        <View style={styles.infoCardAlt}>
          <Text style={styles.infoTitle}>Earnings today</Text>
          <Text style={[styles.infoValue, { color: '#16a34a' }]}>120 000 UZS</Text>
        </View>
      </View>

      <View style={styles.sectionsList}>
        <Pressable style={styles.item} onPress={() => router.push('/(driver)/dashboard')}>
          <ShieldCheck width={18} height={18} />
          <Text style={styles.itemText}>Dashboard</Text>
        </Pressable>
        <Pressable style={styles.item} onPress={() => router.push('/(driver)/map')}>
          <MapPin width={18} height={18} />
          <Text style={styles.itemText}>Map / Zones</Text>
        </Pressable>
        <Pressable style={styles.item} onPress={() => router.push('/(driver)/orders')}>
          <List width={18} height={18} />
          <Text style={styles.itemText}>Orders</Text>
        </Pressable>
        <Pressable style={styles.item} onPress={() => router.push('/(driver)/settings')}>
          <Settings width={18} height={18} />
          <Text style={styles.itemText}>Settings</Text>
        </Pressable>
      </View>

      <Pressable style={styles.helperButton} onPress={() => router.push('/(driver)/support') }>
